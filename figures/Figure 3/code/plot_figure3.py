...
labels = ["CORNN (CPU)", "CORNN (GPU)", "GD-logistic (CPU)", "GD-logistic (GPU)",
          "GD-logistic-fixed point initialized (GPU)",
           "Teacher forced gradient descent  (GPU)", "FORCE (Perich et al 2021)"]

ind_interest = [0,1,2,3,5,6]
col_ind   = [0,0,1,1,2,3,4];
...
for k in ind_interest:
    ...
    if k in [0,2,6]:
        plt.errorbar(np.nanmedian(times_new,0),np.nanmedian(acc_new,0),np.nanstd(acc_new,0),np.nanstd(times_new,0),color = color_all[np.mod(col_ind[k],7)],label = labels[np.mod(k,7)])
    else:
        plt.errorbar(np.nanmedian(times_new,0),np.nanmedian(acc_new,0),
                     np.nanstd(acc_new,0),np.nanstd(times_new,0),
                     color = color_all[np.mod(col_ind[k],7)],
                     label = labels[np.mod(k,7)],ls= '--')
...
plt.savefig("Figure3_python.pdf", bbox_inches='tight')

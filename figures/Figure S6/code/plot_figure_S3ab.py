...
f = np.load('experiment3a_results.npz')
...
temp = acc[0,:,1,[10,0,5,1,6,2,7,3,8,4,9]].T
...
plt.savefig('Figure S3A.pdf', bbox_inches='tight')



#%% Make figure 3b, scaling of output accuracy for CPU

temp = acc[1,:,1,[10,0,5,1,6,2,7,3,8,4,9]].T
...
plt.ylabel('Reconstruction accuracy (r) of $W_{rec}$')
plt.title('Runtime per run is %.1f $\pm$ %.1f seconds' %(np.mean(times_temp),np.std(times_temp)))
plt.savefig('Figure S3b.pdf', bbox_inches='tight')

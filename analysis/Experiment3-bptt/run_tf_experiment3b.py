...
accuracy_rep = np.zeros([2,num_exp,1,num_tr+1]) + np.nan
times =        np.zeros([2,num_exp,1,num_tr])+ np.nan
for k in range(2):
    if k == 0:
        opts = {
            'g': 3,
    ...
    for i in range(num_exp):
        ...
        for j in range(num_tr):
            ...
            if j<5:
                accuracy_rep[k,i,l,j] = run_pytorch_tf(r,u,tf_ratios[j],gnd,'l2',
                                                       initialize_fp = 0,optim_type=optim_tp)
                temp = time_now.localtime()
                current_time = time_now.strftime("%H:%M:%S", temp)
                print('\t \t %s: Loss Type l2. TF ratio %.1f. Opt. type %s. Accuracy %.3f.' %(current_time,
                           tf_ratios[j],optim_tp,accuracy_rep[k,i,l,j]))
            else:
                accuracy_rep[k,i,l,j] = run_pytorch_tf(r,u,tf_ratios[j],gnd,'bce',
                                                       initialize_fp = 0,optim_type=optim_tp)
                temp = time_now.localtime()
                current_time = time_now.strftime("%H:%M:%S", temp)
...
np.savez('experiment3b_results.npz',accuracy_rep = accuracy_rep, times = times)

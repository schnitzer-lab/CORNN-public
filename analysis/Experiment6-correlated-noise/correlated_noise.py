...
def gaussian_kernel(sigma_x, sigma_y,size = 30):
...
def generate_noise(n_rec,T,sigma = 1e-3,correlated = True,sigma_N = 5,sigma_T = 3):
    
    correlated_noise = np.random.normal(0, sigma, size = (T,n_rec) )
    if correlated == True:
        if sigma >0:
            gaussian_kernel_2d = gaussian_kernel(sigma_T,sigma_N)
            correlated_noise = convolve(correlated_noise, gaussian_kernel_2d, mode='nearest')
            correlated_noise = correlated_noise * sigma / np.std(correlated_noise)
...
class Model(nn.Module):
    ...
    def forward(self, u, r,noise_cor = 0,corrf = True):
        ...
        noise = generate_noise(n_rec,T,noise_cor,corrf);
...
cor_weights_rec     = np.zeros([num_exps,2,num_trials.shape[0],n_sup_all.shape[0]])
cor_weights_in      = np.zeros([num_exps,2,num_trials.shape[0],n_sup_all.shape[0]])
r2_units            = np.zeros([num_exps,2,num_trials.shape[0],n_sup_all.shape[0]])
r2_units_dist       = np.zeros([num_exps,2,num_trials.shape[0],n_sup_all.shape[0]])
times               = np.zeros([num_exps,2,num_trials.shape[0],n_sup_all.shape[0]])
alph = 0.1 
for cor_flag in range(2):
    if cor_flag == 0:
        corrf = True
    else:
        corrf = False
    for kk in range(num_exps):
        ...
        for ss in range(n_sup_all.shape[0]):
            ...
            for i in range(100):
                ...
                # forward
                h,out = model(u,r_init,noise_cor,corrf)
            ...
            for ll in range(num_trials.shape[0]):
                ...
                times[kk,cor_flag,ll,ss] = time_now.perf_counter()-start_time;
                ...
                cor_weights_rec[kk,cor_flag,ll,ss] = pearsonr(gnd,prd)[0]
                ...
                cor_weights_in[kk,cor_flag,ll,ss] = pearsonr(gnd_in.flatten(),prd_in)[0]
                ...
                r2_units[kk,cor_flag,ll,ss] = np.mean(r2_temp)
                r2_units_dist[kk,cor_flag,ll,ss] = np.mean(r2_dist_temp)
                
                temp = time_now.localtime()
                current_time = time_now.strftime("%H:%M:%S", temp)
                print('%s: Correlated %d. Experiment %d. Num Trials %d. Noise %f. Time: %.2f. Input %.2f. Rec %.2f. R2 %.2f and %.2f.' %(current_time,cor_flag, 
                                                             kk,n_trial,noise_cor ,times[kk,cor_flag,ll,ss],cor_weights_in[kk,cor_flag,ll,ss]
                                                             , cor_weights_rec[kk,cor_flag,ll,ss],r2_units[kk,cor_flag,ll,ss],
                                                             r2_units_dist[kk,cor_flag,ll,ss]))

np.savez('correlated_noise_experiment.npz',r2_units = r2_units,

...
opts = {};
opts['g'] = 3; 
opts['n_rec'] = 500 # 300 for short
opts['n_in'] = 1
opts['sigma_input'] = 1e-1
opts['sigma_conversion'] = 1e-5
opts['alpha'] = 0.1
...
opts['num_cores'] = 4
opts['parallel'] = 1
T_data = 10000; # 3000 for short


num_algs = 9;
num_exps = 100;
iter_list = np.linspace(0,10,11).astype(int)
    ...
    for iters in iter_list:
        ...
        if iters == 0:
            try:
                w  = solve_newton_descent(r[:-1,:],r[1:,:],u_in = None,alph = 0.1,
                                     l2 = 1e-5,verbose = 0,threshold =0.2,
        ...
        if counter[3] == 0:
            try:
                ...
                print('\t \t \t NT,weighted. Cor %.3f. %.1f mins' %(cor_p[3,idx_exp,iters],\
                                                                ( time_now.perf_counter() - start_time)/60    ))
        ...
        if counter[5] == 0: 
            try:
                ...
                print('\t \t \t NT,logistic. Cor %.3f. %.1f mins' %(cor_p[3,idx_exp,iters],\
                                                ( time_now.perf_counter() - start_time)/60    ))
                if time_now.perf_counter() - start_time > time_limit:
                    counter[5] =1
            except:
        ...
        if counter[0] == 0:
            try:
                ...
                w = solve_corrn(r[:-1,:],r[1:,:],u_in = None,l2 = 1*1e-5,
                                verbose = 0,gnd = gnd, threshold = 1, initialize_fp = 0,
                                num_iters = 300*iters,check_convergence = 0,
                                solver_type = 'weighted')
        ...
        if iters == 0:
            w = solve_corrn_gpu(r[:-1,:],r[1:,:],u_in = None,l2 = 1*1e-5,
                            ...
                            num_iters = 500*iters, float_type = '64bit',
                            solver_type = 'weighted')
        start_time = time_now.perf_counter()
        if counter[8] == 0:
            try:
                ...
                w = solve_corrn_gpu(r[:-1,:],r[1:,:],u_in = None,l2 = 1*1e-5,
                                verbose = 0,gnd = gnd, threshold = 1, initialize_fp = 0,
                                num_iters = 300*iters, float_type = '32bit',
                                solver_type = 'weighted')
        ...
        if counter[6] == 0:
            try:
                ...
                print('\t \t \t CoRNN, CPU. Cor %.3f' %(cor_p[6,idx_exp,iters]))
                if time_now.perf_counter() - start_time > time_limit:
                    counter[6] =1
            except:
                print('\t \t \t CoRNN, CPU terminated due to timeout error')
        ...
        if counter[7] == 0:
            try:
                ...
                w = solve_corrn_gpu(r[:-1,:],r[1:,:],u_in = None,l2 = 1*1e-5,
                                verbose = 0,gnd = gnd, threshold = 1, initialize_fp = 1,
                                num_iters = 5*iters, float_type = '32bit',
                                solver_type = 'weighted')
                ...
                print('\t \t \t CoRNN, GPU. Cor %.3f' %(cor_p[7,idx_exp,iters]))
                if time_now.perf_counter() - start_time > time_limit:
                    counter[7] =1
            except:
                print('\t \t \t CoRNN, GPU terminated due to timeout error')
        ...
        if counter[1] == 0:
            try:
                ...
                w  = solve_pytorch_gpu(r[:-1,:],r[1:,:],u_in = None,alph = 0.1,
                                         ...
                                         initialize_fp =0,num_iters = iters*500,
        ...
        if iters == 0:
            counter = np.zeros(num_algs) # Force takes hours to run iteration 1 ....


np.savez('experiment1c_results_fp.npz',slopes = slopes,

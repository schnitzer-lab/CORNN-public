...
class CustomRNN(nn.Module):
    ...
    def forward_noisy(self, x, h,conv_noise = 1e-3,inp_noise=1e-2):
        """
        The shape of x is (batch_size, seq_len, input_dims)
        The shape of h is (batch_size, hidden_dims)
        The shape of output is (batch_size, seq_len, output_dims)
        ...
        for t in range(x.size(1)):
            ...
            h_in = torch.einsum("ij,jk->ik", x[:, t, :], self.W_in)
            h = h_from_prev + self.alpha * torch.tanh(h_rec + h_in + \
                                    torch.empty(n_rec).normal_(0,inp_noise)) +self.alpha* torch.empty(n_rec).normal_(0,conv_noise)
            hidden_states.append(h)
    ...
    def run_rnn(self, inputs, outputs, device="cpu",noisy = 0,conv_noise = 1e-3,inp_noise = 1e-2):
        ...
        if noisy == 0:
            ...
            output, h = self.forward_noisy(x, h,conv_noise,inp_noise)
...
class K_bit_flip_flop():
    ...
    def gen_trial(self,tsp = 0,T_in = None):
        if T_in is not None:
            T_old = self.T
            self.T = T_in

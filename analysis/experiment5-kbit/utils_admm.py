...
def approximate_newton(r_in,r_out,u_in = None, alph = 0.1, 
    ...
    for idx in range(10):
        ...
        x2 = np.mean(x,0)
        Xp   = Ainv @ x2;
        times = time_now.perf_counter() - start_time + times
    
    times =n_rec * times/10;
    
   
    return times

def solve_corrn_admm(r_in,r_out,u_in = None, alph = 0.1, 
                l2 = 1e-4, threshold = 1, rho = 10,
    ...
    for idx in range(num_iters):
        ...
        if ((verbose == 2) & (np.mod(idx,round(num_iters/10)) == np.mod(-1,round(num_iters/10)) )):
            temp = time_now.localtime()
            current_time = time_now.strftime("%H:%M:%S", temp)
            slope = (gnd @ prd) / (gnd@gnd) 
